...
//        }
        ...
        $events = \App\Event::orderBy('events.created_at', 'DESC')
            ->join('users', 'users.id', '=', 'events.user_id')
            ->join('books', 'books.id', '=', 'events.book_id')
            ->join('authors', 'authors.id', '=', 'books.author_id')
            ->select('events.id', 'books.title', 'authors.author_name', 'events.action', 'users.name')
            ->limit(5)->get();

        return view('index', compact('books', 'events'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $action = 'add';
        $event_data = [
            'book_id' => $book_id, 'user_id' => $user_id, 'action' => $action
        ];

        event(new NewBookAddedEvent($event_data));
    ...
    public function book($book_id)
    {
        ...
        event(new BookIsBookedEvent($event_data));
